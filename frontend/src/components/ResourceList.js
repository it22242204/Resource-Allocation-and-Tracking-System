    ...
    try {
      ...
      const updatedResources = response.data.map((resource) => {
        if (
          ...
          // Push API update promises to the array
          updates.push(
            axios.put(`http://localhost:5000/resources/${resource.id}/status`, {
              status: "Available",
            }),
            axios.put(`http://localhost:5000/allocations/${resource.id}/status`, {
              status: "Available",
            })
          );
  ...
  useEffect(() => {
    ...
    const pollResources = () => {
      ...
      fetchResources().finally(() => {
        setTimeout(pollResources, 60000); // Wait for completion before scheduling the next
      });
    };
    ...
    <div className="min-h-screen bg-gradient-to-b from-blue-50 to-white text-gray-800">
      <header className="bg-gradient-to-r from-blue-600 to-indigo-600 text-white py-12 shadow-lg">
        <div className="max-w-7xl mx-auto px-6 text-center">
          <h1 className="text-4xl font-bold mb-4">Welcome to the Resource Allocation System</h1>
      ...
      >
        <h2 className="text-3xl font-bold mb-8 text-center text-blue-700 drop-shadow-lg">Available Resources</h2>
        ...
        {resources.length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
            {resources.map((resource) => (
              ...
              >
                <h3 className="text-xl font-semibold mb-2 text-blue-600 tracking-wide">{resource.name}</h3>
                <p className="text-gray-600 text-sm mb-4 italic">{resource.description || "No description available."}</p>
                <p
